...
确认文件已成功上传后再调用 confirmUpload，如预签名 URL 已过期请重新申请
...
服务端检测到上传文件的 sha256 与客户端声明不一致，拒绝激活该文件记录

**常见触发原因**

- 上传过程中文件内容被篡改
- 客户端计算 sha256 时使用了错误的算法或编码

**排查建议**

重新计算文件 sha256（小写十六进制）后重新上传
