...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <Button
                class="gap-2 bg-bsb-accent-brand text-white hover:bg-bsb-accent-hover"
                @click="router.push('/box/new')"
            >
                <Plus class="size-4" />
                新建
            </Button>
        ...
        <div class="relative max-w-sm">
            ...
            <Input
                v-model="keyword"
                placeholder="搜索储存盒…"
                class="border-bsb-border-standard bg-bsb-bg-surface pl-10 text-bsb-text-primary placeholder:text-bsb-text-quaternary"
                @input="handleSearch"
            />
        </div>
        ...
        <template v-if="isSearching">
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <Card
                    v-for="box in searchResults"
                    :key="box.id"
                    class="box-card cursor-pointer border-bsb-border-standard bg-bsb-bg-panel transition-colors hover:border-bsb-accent-brand/30"
            ...
            <p v-if="searchResults.length === 0" class="text-sm text-bsb-text-quaternary">
                未找到匹配的储存盒
            </p>
        </template>
        ...
        <template v-else>
            ...
            <p v-if="groups.length === 0" class="text-sm text-bsb-text-quaternary">
                暂无储存盒，点击"新建"创建第一个
            </p>
        </template>
    </div>
