...
本文档是 **BSB-Frontend** 工作区的 AI 助手操作手册，继承根目录 `AGENTS.md` 的通用规范，在此基础上扩展前端专属的核心原则、上下文获取路径与验证流程。
...
- [ ] `pnpm --filter BSB-Frontend type-check` — 零错误
...
pnpm --filter BSB-Frontend build         # 构建产物（含类型检查）

# 代码质量
pnpm --filter BSB-Frontend eslint        # ESLint 自动修复
pnpm --filter BSB-Frontend prettier      # Prettier 格式化
```
...
- [BSB-Backend AGENTS.md](../BSB-Backend/AGENTS.md) — 后端操作手册（接口结构、DTO）
